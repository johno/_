...
export const regenColorWithMemory = color => {
  ...
  const blend = () => chroma.blend(color, randomHexColor(), sample(blendAlgorithms))
  const adjustments = [
    '*1.5',
    '*1.25',
    '*1.1',
...
export const RandomEvolvingColors = ({ count = 50 }) => {
  ...
  const els = colors.map((color, i) => <div key={i} style={{ height: 16, width: '2%', backgroundColor: color }} />)
  return (
    <div onClick={regenColors} style={{ cursor: 'pointer', display: 'flex', flexWrap: 'wrap' }}>
      {els}
    </div>
  )
}
...
export const RandomEvolvingColorsComparison = ({ count = 50, width= '2%', height = 16 }) => {
  const [colors, setColors] = React.useState([])
  ...
  const avgDistance = colors => {
    const sumOfDistance = colors.map((color, i) => {
      if (!colors[i+1]) {
        return null
      }
      return chroma.distance(color, colors[i+1])
  ...
  const els = colors.map((color, i) => <div key={i} style={{ height, width, backgroundColor: color }} />)
  const randEls = randColors.map((color, i) => <div key={i} style={{ height, width, backgroundColor: color }} />)
  return (
    <div onClick={regenColors} style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: -20 }}>
      <div style={{ width: '45%' }}>
        <h3>Memory-based</h3>
        <div style={{ cursor: 'pointer', display: 'flex', flexWrap: 'wrap' }}>
          {els}
        </div>
        <h5 style={{ marginTop: 10 }}>Average color distance: {avgDistance(colors)}</h5>
      </div>
      <div style={{ width: '45%' }}>
        <h3>Pure random</h3>
        <div style={{ cursor: 'pointer', display: 'flex', flexWrap: 'wrap' }}>
        ...
        <h5 style={{ marginTop: 10 }}>Average color distance: {avgDistance(randColors)}</h5>
      </div>
    </div>
  )
}
...
export const RandomEvolvingColorsSlider = () => {
  ...
  const regenColor = () => {
    const newColor = randomBool(entropy / 100) ? regenColorWithMemory(color) : regenColorWithMemory()
    setColor(newColor)
  }
  return (
    <div>
      <div
        onClick={regenColor}
        style={{ height: 240, width: 240, cursor: 'pointer', display: 'flex', flexWrap: 'wrap', backgroundColor: color }}
      />
...
Thanks to Adam Morse, Chris Biscardi, Brent Jackson, Cole Bemis, Taylor Bell,
Florian Kissling, Prince Wilson, Rose Wiegley, Nick Bender, and my wife for
