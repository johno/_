...
Composition over configuration is a key tenet in [MDX](https://mdxjs.com)
and where it departs from existing Markdown approaches. It allows authors
to import and mix in functionality (as a component) rather than overloading
Markdown syntax.

Not to mention, syntax overloading is typically done at the build tool level
which means a plugin that's intended to target a handful of pages happens
across the board.
...
avoids framework coupling. Another benefit is the fact that composition
becomes more flexible and explicit.
...
composition and how it relates to components in React/[MDX]().

First, let's consider where configuration falls short.
...
JSX and components provide an expressive syntax for composition
...
When there's a common usage, a composition can be tucked away.

```js
<ImageWithCitation src="12345" alt="numbers" photography="Fred Flintstone" />
```

It's important to note that `ImageWithCitation` is a contrived example
that is better expressed as two separate, sibling components.
...
This allows small changes to be made in one place that will be reflected
in all call sites for the component.
...
If `CustomYoutube` component consists of `ResponsiveIframe` and `Youtube`
you can break away from the composite component to add one-off usage.
