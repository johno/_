...
When you apply specific naming to an idea or concept you can override it with
more specificity. This breaks down as colors evolve.
...
Libraries like Tachyons, Basscss, and Styled System introduced the concept of
...
so consistency is maintained for most scenarios.
...
quickly make tweaks that adhere to a larger design system.

### Consistency

These scales should persist throughout a design system wherever relevant.
...
expected to be known by end users.

Gatsby Themes introduced a concept known as component shadowing and I
...
Now, in place of the theme's AuthorBio component is overridden with the
user's.
...
creating components. Gatsby users are already working out of the `src`
directory, so the desire to override a theme's component becomes
...
you're empowered to give it a try. Empowerment to give it a try means that the
tooling ensures that this is efficient. That includes live reloading, source
maps, and projects like React Devtools that improve the debugging experience.

This inspires confidence in the system because you're not punished for guessing.

When a designer or developer knows a change will show up in the browser in 1s,
...
reading the source code of a component.
...
It removes the need for guessing because default behaviors are built in.

### It's not perfect

I wouldn't call `yarn` perfect either. Some of it's guessability is lost when you
consider it's attempt to be a replacement for `npm`. Commands are subtly different,
...
Theme Specification that seeks to standardize the commonalities in a design
system's `theme.js`. When a community arrives at a common spec, everyone
benefits.
