...
them to render in child components. At the time, it seemed
like a unique need for MDX, since it made sense to declare
...
To the user it means: **"when you an encounter an h1 in MDX, render
MyH1 instead"**.
...
It's akin to the following CSS:

```css
.purple-h1s h1 { color: rebeccapurple; }
```
...
in interactivity or swap out `h1`s for `h2`s in nested documents to
ensure proper semantic markup hierarchies.
...
because **prism output can sometime 10x DOM output when rendered to markup**.
Granted, this is only the case after rehydration, but that's a fine tradeoff
for content-heavy sites with multi-page visits.
